...
public class MeshExporterBin : Editor
{
    ...
    private static string s_textureExportDir = "";  // .bin 옆 Textures/ 폴더 경로
    ...
    static void ExportToBinary()
    {
        ...
        // 텍스처 복사 폴더 준비
        s_textureExportDir = Path.Combine(Path.GetDirectoryName(path), "Textures");
        ...
        for (int i = 0; i < materials.Length; i++)
        {
            ...
            if (materials[i].HasProperty("_MainTex"))
            {
                ...
                if (pTexture)
                {
                    ...
                    {
                        string texFileName = CopyTexture(assetPath);
                        WriteString("<AlbedoMap>:", texFileName);
                    }
                }
            }
    ...
    {
        if (s_exportedTextures.TryGetValue(assetPath, out string cached))
            return cached;
        ...
        if (!File.Exists(destPath))
        {
            if (needConvert)
            {
                ...
                if (tex != null)
                {
                    var readable = MakeTextureReadable(tex);
